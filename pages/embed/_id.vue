...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getStreamData () {
        ...
        this.streamDataListener = db
          ...
          */
         console.log("Should trigger the stream data fetch, would be nice to hook this up to use the webrtc stuff or other rtmp streams...")
      },
    ...
    async asyncData ( { $axios, params, query } ) {
      ...
      try {
        const { data } = await $axios.getSSR( ``+ process.env.APISERVER +`api/channel/${user}`, { timeout } );
        ...
        let   url    = data.url;
        console.log( `Looking for stream info ${user}: ${url}` );
