  ...
  export default {
    ...
    methods: {
      ...
      connectChat () {
        ...
        this.socket = socketio( ''+ process.env.APISERVER +'', { transports: [ 'websocket' ] } );
      ...
      async httpHydrate () {
        ...
        const getChatHydration = async ( channel, global ) => {
          try {
            ...
            const { data } = await this.$axios.getSSR( ``+ process.env.APISERVER +`v1/messages${ global ? '' : `/${channel}` }`, { timeout } );
            console.log("Hydration testing:",data);
