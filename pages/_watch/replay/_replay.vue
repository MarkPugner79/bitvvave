  ...
  export default {
    ...
    async asyncData ( { $axios, params, error } ) {
      ...
      const getReplayHydration = async id => {
        ...
        try {
          const { data } = await $axios.getSSR( `` + process.env.APISERVER + `v1/replays/${id}`, { timeout } );
    ...
    methods: {
      ...
      async trackReplayView () {
        const url = `` + process.env.APISERVER + `v1/replays/${this.archiveId}/watch`;
        try {
      ...
      async streamDataChanged ( data ) {
        ...
        // Fix relative urls...
        let szTempUrl = data.url;
        // Probably we don't need to check it against https or http, simple check is enough
        szTempUrl = szTempUrl.startsWith( 'http' ) ? szTempUrl : `https://${szTempUrl}`;
