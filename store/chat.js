...
export const actions = {
  ...
  async [$actions.updateEmoteMap] ( { state, commit } ) {
    ...
    try {
      const { data } = await this.$axios.get( ''+ process.env.APISERVER +'v1/emotes', { progress: false } );
  ...
  async [$actions.exchangeIdTokenChatToken] ( { dispatch }, idToken ) {
    try {
      const { data } = await this.$axios.post( ``+ process.env.APISERVER +`v1/user/exchangetoken`, { token: idToken }, { progress: false } );
      await dispatch( $actions.updateChatToken, data );
  ...
  async [$actions.createTrollToken] ( { dispatch } ) {
    const { data } = await this.$axios.get( ''+ process.env.APISERVER +'trollid', { progress: false } );
    await dispatch( $actions.updateChatToken, data );

    // TODO: Can this be replaced with saveToLocalStorage
    try {
      //localStorage.setItem( 'troll', data.chatToken );
      localStorage.setItem( 'troll', data );
    } catch ( error ) {
