    ...
    if ( !throttledUpdateViewers ) {
      ...
      const update = async () => {
        const updateChannelViewers = async () => {
          try {
            const { data } = await this.$axios.get( '' + process.env.APISERVER + 'v1/chat/channels', { progress: false, skipAuth: true } );
        ...
        const updateUserList = async () => {
          try {
            const { data } = await this.$axios.get( '' + process.env.APISERVER + 'v1/chat/users', { progress: false, skipAuth: true } );
            if ( data && data.success ) {
              console.log("User list from server:",data.data);
