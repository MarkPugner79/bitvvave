...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      async liveStreamers () {
        try {
          const { data } = await this.$axios.get( '' + process.env.APISERVER + 'v1/channels/live' );
    ...
    async mounted () {
      ...
      //setInterval(function(){ this.scrollToBottom();},1000);
      // would be nice to see the last time a message came in and if we want this to fire to force it to bottom or not, or only 
      // this needs a global and prevents this from firing if the mouse is over the chat window
      //this.statInterval = setInterval( () => this.scrollToBottom(), 1000 );
      // or make this a toggle option in the settings

      // viewer list viewcount FIXME
