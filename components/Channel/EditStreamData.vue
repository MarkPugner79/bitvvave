      ...
      async updateStreamData () {
        ...
        */
        ...
        if ( this.live && this.archive !== this.streamData.archive ) {
          const endpoint = `` + process.env.APISERVER + `v1/streamer/recorder/${this.streamData.archive ? 'start' : 'stop' }`;
      ...
      async uploadFile () {
        ...
        try {
          const { data } = await this.$axios.post(
            '' + process.env.APISERVER + 'v1/upload/cover',
            formData,
      ...
      async deleteOldCover () {
        try {
          const { data } = await this.$axios.delete(
            '' + process.env.APISERVER + 'v1/upload',
            {
