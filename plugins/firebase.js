...
// strip out all of the firebase garbage
...
export default async ( { app, store, $axios, $sentry }, inject ) => {
  ...
  if ( process.client ) {
    ...
    console.log("Removed Performance crap, stripping out firebase...");

    // Inject analytics into context
    console.log( `Starting and injecting analytics module.` );
    console.log("Removed Analytics crap, stripping out firebase...");
    /*
      ...
      if ( process.client ) {
        ...
        const processToken = async ( token ) => {
          try {
            ...
            if ( parseToken( token ).exp - Date.now() / 1000 <= ( 5 * 60 ) ) {
